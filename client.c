...
{
    ft_putstr("whats going on\n");
    client->ret_from_server = recv(client->client_socket, buff, 2048, 0);
    printf("%d\n", client->ret_from_server);
    ft_putstr("helloioo\n");
...
void    open_read(int fd, char **store, t_client *client)
{
    ...
    if ((fd = open(file, O_RDONLY)) <= 0)
        error("Couldn't open file.\n");
    // fstat gets the file info 
    fstat(fd, &st);
    //sending the file size as an 8 byte int
